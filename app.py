...
import numpy as np
import cv2
from gradcam import get_gradcam_heatmap, overlay_gradcam, get_last_conv_layer
...
def inject_styles():
    ...
    <style>
        ...
        section[data-testid="stSidebar"] h3 {{ color: {TEXT_PRIMARY} !important; }}
        header[data-testid="stHeader"] {{ background: transparent; }}
        footer {{ visibility: hidden; }}
        h1,h2,h3,h4,h5,h6 {{ color: {TEXT_PRIMARY} !important; }}
        p,li,span,div,label {{ color: {TEXT_SECONDARY}; }}
        [data-testid="stFileUploader"] {{ background-color: transparent !important; }}
        [data-testid="stFileUploader"] section {{
            background-color: {BG_SECONDARY} !important; border: 2px dashed {BORDER} !important;
            border-radius: 16px !important; padding: 40px 20px !important; transition: border-color 0.2s;
        }}
        [data-testid="stFileUploader"] section:hover {{ border-color: {ACCENT} !important; }}
        [data-testid="stFileUploader"] section * {{ color: {TEXT_SECONDARY} !important; }}
        [data-testid="stFileUploader"] button {{
            background-color: {ACCENT} !important; color: {BG_PRIMARY} !important;
            border: none !important; border-radius: 8px !important; font-weight: 600 !important;
        }}
        [data-testid="stFileUploaderFile"] {{
            background-color: {BG_SECONDARY} !important; border: 1px solid {BORDER} !important; border-radius: 8px !important;
        }}
        .stTabs [data-baseweb="tab-list"] {{
            gap: 4px; background-color: {BG_SECONDARY}; border-radius: 10px; padding: 4px;
        }}
        .stTabs [data-baseweb="tab"] {{
            background-color: transparent; border: none; border-radius: 8px;
            color: {TEXT_MUTED} !important; padding: 8px 16px; font-size: 14px;
        }}
        .stTabs [aria-selected="true"] {{
            background-color: {BG_CARD} !important; color: {ACCENT} !important; border: 1px solid {BORDER} !important;
        }}
        .card {{
            background-color: {BG_CARD}; border: 1px solid {BORDER}; border-radius: 16px; padding: 24px;
        }}
        .card-inner {{
            background-color: {BG_SECONDARY}; border: 1px solid {BORDER}; border-radius: 12px; padding: 16px;
        }}
        .feature-card {{
        ...
        .feature-card:hover {{ border-color: rgba(45,212,191,0.25); }}
        .step-num {{ font-family: monospace; font-size: 11px; color: rgba(45,212,191,0.55); font-weight: 700; }}
        .symptom-item {{
            display: flex; align-items: center; gap: 12px;
            background-color: {BG_SECONDARY}; border-radius: 10px; padding: 12px 16px; margin-bottom: 8px;
        }}
...
TUMOR_INFO = {
    "glioma": {
        "label": "Glioma", "color": "#ef4444", "severity": "High Concern", "severity_level": "high",
        "description": "Gliomas originate from glial cells in the brain or spine. They can be aggressive and may require immediate treatment.",
        ...
        "prevalence": "~33% of all brain tumors", "survival_rate": "5-year survival varies by grade",
    },
    "meningioma": {
        "label": "Meningioma", "color": "#f59e0b", "severity": "Moderate Concern", "severity_level": "moderate",
        "description": "Meningiomas arise from the meninges. They are often benign and slow-growing.",
        ...
        "prevalence": "~30% of all brain tumors", "survival_rate": "~80% five-year survival",
    },
    "notumor": {
        "label": "No Tumor", "color": "#22c55e", "severity": "All Clear", "severity_level": "clear",
        "description": "No tumor detected. Brain structure appears normal. Regular check-ups still recommended.",
        "symptoms": ["No concerning indicators detected"],
        "treatment": ["Routine neurological check-ups", "Healthy lifestyle maintenance"],
        "prevalence": "N/A", "survival_rate": "N/A",
    },
    "pituitary": {
        "label": "Pituitary", "color": "#38bdf8", "severity": "Requires Attention", "severity_level": "moderate",
        "description": "Pituitary tumors occur in the gland at the base of the brain, often affecting hormone production.",
        "symptoms": ["Hormonal imbalance", "Vision disturbance", "Chronic headaches", "Fatigue", "Weight changes"],
        "treatment": ["Hormone therapy", "Transsphenoidal surgery", "Medication (Cabergoline)", "Radiation therapy"],
        "prevalence": "~17% of all brain tumors", "survival_rate": "~95% five-year survival",
    },
}
...
def real_prediction(image_bytes):
    ...
    processed = preprocess_input(np.copy(img_array))
    ...
    probs = {CLASS_NAMES[i]: round(float(preds[i] * 100), 2) for i in range(len(CLASS_NAMES))}
    top_class = CLASS_NAMES[np.argmax(preds)]
    confidence = probs[top_class]

    last_conv_layer_name = get_last_conv_layer(model)
    if last_conv_layer_name is None:
        print("[GradCAM] Could not find a suitable conv layer.")
        orig_b64 = heat_b64 = over_b64 = None
    else:
        try:
            orig_img_array = np.array(image.resize((224, 224)))
            heatmap = get_gradcam_heatmap(processed, model, last_conv_layer_name)
            if heatmap is None:
                orig_b64 = heat_b64 = over_b64 = None
            else:
                pure_heatmap, overlay = overlay_gradcam(orig_img_array, heatmap)
                ...
                orig_b64 = img_to_b64(orig_img_array)
                heat_b64 = img_to_b64(pure_heatmap)
                over_b64 = img_to_b64(overlay)
        except Exception:
            import traceback
            traceback.print_exc()
            orig_b64 = heat_b64 = over_b64 = None
    ...
    return {
        ...
        "probabilities": probs,
        "time": analysis_time,
        "images": (orig_b64, heat_b64, over_b64)
    }
...
def create_donut_chart(probs):
    ...
    fig.update_layout(
        ...
        font=dict(color=TEXT_SECONDARY), showlegend=True,
        legend=dict(font=dict(color=TEXT_SECONDARY, size=12), bgcolor="rgba(0,0,0,0)",
...
def create_gauge(value, color, label):
    fig = go.Figure(go.Indicator(
        ...
        gauge=dict(
            ...
            bar=dict(color=color, thickness=0.7), bgcolor=BG_SECONDARY,
            bordercolor=BORDER, borderwidth=1,
            steps=[dict(range=[0,40], color=BORDER), dict(range=[40,70], color=BG_SECONDARY), dict(range=[70,100], color=BORDER)],
            threshold=dict(line=dict(color=color, width=3), thickness=0.8, value=value),
        ),
    ))
    fig.update_layout(
        plot_bgcolor="rgba(0,0,0,0)", paper_bgcolor="rgba(0,0,0,0)",
        font=dict(color=TEXT_SECONDARY), margin=dict(l=30, r=30, t=30, b=10), height=220,
    )
    return fig
...
def create_radar_chart(probs):
    ...
    fig.add_trace(go.Scatterpolar(
        ...
        fill="toself", fillcolor="rgba(45,212,191,0.1)",
    ...
    fig.update_layout(
        polar=dict(
            ...
            angularaxis=dict(gridcolor=BORDER, tickfont=dict(color=TEXT_SECONDARY, size=12)),
        ),
        plot_bgcolor="rgba(0,0,0,0)", paper_bgcolor="rgba(0,0,0,0)",
        font=dict(color=TEXT_SECONDARY), showlegend=False,
...
def render_sidebar():
    with st.sidebar:
        ...
        for k, v in {"Architecture":"EfficientNet-B3","Input Size":"300×300 px","Parameters":"~5.3M","Training Acc.":"87.5%","Framework":"TensorFlow"}.items():
            st.markdown(f"""
...
def render_hero():
    ...
    <div style="text-align:center; padding:20px 0 10px;">
        ...
        <p style="color:{TEXT_MUTED}; font-size:16px; max-width:560px; margin:12px auto 0; line-height:1.6;">
            ...
            across 4 tumor types with Grad-CAM explainability.
        </p>
    </div>
    """, unsafe_allow_html=True)
...
def render_features():
    features = [
        ...
        ("🔥", "Grad-CAM XAI", "Visual heatmaps showing exactly what the AI focused on"),
...
def render_results(result, image_bytes):
    ...
    orig_b64, heat_b64, over_b64 = result["images"]

    st.markdown(f"""
    <h2 style="font-size:22px; font-weight:700; color:{TEXT_PRIMARY} !important; margin:0 0 20px;">Analysis Results</h2>
    """, unsafe_allow_html=True)

    # ── Row 1: uploaded image / diagnosis / gauge ──
    col_img, col_diag, col_gauge = st.columns([1, 1.2, 1])

    with col_img:
        st.markdown(f"""
        <div class="card" style="padding:16px;">
            <div style="background-color:rgba(0,0,0,0.4); border-radius:12px; overflow:hidden; text-align:center; position:relative;">
                ...
                <div style="position:absolute; top:12px; right:12px;
                            ...
                            padding:4px 12px; font-size:11px; font-weight:700;">Analyzed</div>
            </div>
        </div>""", unsafe_allow_html=True)
    ...
    with col_diag:
        ...
        <div class="card">
            ...
            <div style="display:flex; align-items:center; gap:8px; margin-bottom:12px;">
                ...
                <span style="border:1px solid {info['color']}; color:{info['color']}; padding:4px 12px; border-radius:999px; font-size:11px; font-weight:600;">{info['severity']}</span>
            </div>
            <p style="font-size:13px; color:{TEXT_MUTED}; line-height:1.6; margin-bottom:16px;">{info['description']}</p>
        </div>""", unsafe_allow_html=True)
    ...
    with col_gauge:
        ...
        <div style="display:flex; gap:12px; margin-top:-10px;">
            <div class="card-inner" style="flex:1; text-align:center;">
                <p style="font-size:11px; color:{TEXT_MUTED}; margin:0;">Time</p>
                <p style="font-size:13px; color:{TEXT_PRIMARY}; font-weight:700; margin:4px 0 0;">{result['time']:.2f}s</p>
            </div>
            <div class="card-inner" style="flex:1; text-align:center;">
                <p style="font-size:11px; color:{TEXT_MUTED}; margin:0;">Model</p>
                <p style="font-size:13px; color:{TEXT_PRIMARY}; font-weight:700; margin:4px 0 0;">EffNetB3</p>
            </div>
        </div>""", unsafe_allow_html=True)

    # ── Row 2: Grad-CAM images ──
    st.markdown("<br>", unsafe_allow_html=True)
    st.markdown(f"<h3 style='font-size:16px; font-weight:600; color:{TEXT_PRIMARY} !important; margin-bottom:12px;'>🔥 Explainability — Grad-CAM Visualization</h3>", unsafe_allow_html=True)

    def img_card(title, img_b64, footer):
        if img_b64 is None:
            return f"""
            <div class="img-card" style="min-height:200px; justify-content:center;">
                <p style="font-size:13px; font-weight:600; color:{TEXT_SECONDARY}; margin-bottom:8px;">{title}</p>
                <p style="font-size:12px; color:{TEXT_MUTED};">⚠️ Unavailable — check terminal</p>
            </div>"""
        return f"""
        <div class="img-card">
            <p style="font-size:13px; font-weight:600; color:{TEXT_SECONDARY}; margin-bottom:10px;">{title}</p>
            <img src="data:image/png;base64,{base64.b64encode(img_b64).decode()}"
                 style="width:100%; border-radius:8px; object-fit:contain; max-height:220px;" />
            <p style="font-size:11px; color:{TEXT_MUTED}; margin-top:10px;">{footer}</p>
        </div>"""

    gc1, gc2, gc3 = st.columns(3)
    with gc1: st.markdown(img_card("Original MRI", orig_b64, "224×224 px"), unsafe_allow_html=True)
    with gc2: st.markdown(img_card("Grad-CAM Heatmap", heat_b64, "Low ── Attention ── High"), unsafe_allow_html=True)
    with gc3: st.markdown(img_card("Overlay", over_b64, f"<span style='color:{info['color']};'>🔥 High-attention region</span>"), unsafe_allow_html=True)

    # ── Row 3: Charts ──
    st.markdown("<br>", unsafe_allow_html=True)
    col_radar, col_donut = st.columns(2)
    with col_radar:
        st.markdown(f"<div class='card' style='padding:20px 20px 8px;'><h3 style='font-size:15px; font-weight:600; color:{TEXT_PRIMARY} !important; margin:0 0 12px;'>Radar Analysis</h3></div>", unsafe_allow_html=True)
        st.plotly_chart(create_radar_chart(probs), use_container_width=True, config={"displayModeBar": False})
    with col_donut:
        st.markdown(f"<div class='card' style='padding:20px 20px 8px;'><h3 style='font-size:15px; font-weight:600; color:{TEXT_PRIMARY} !important; margin:0 0 12px;'>Distribution</h3></div>", unsafe_allow_html=True)
        st.plotly_chart(create_donut_chart(probs), use_container_width=True, config={"displayModeBar": False})

    # ── Row 4: Clinical tabs ──
    st.markdown("<br>", unsafe_allow_html=True)
    tab_s, tab_t, tab_st = st.tabs(["🩺 Symptoms", "💊 Treatment", "📊 Statistics"])
    with tab_s:
        cols = st.columns(2)
        for i, s in enumerate(info["symptoms"]):
            with cols[i % 2]:
                st.markdown(f"""<div class="symptom-item"><div class="dot" style="background-color:{info['color']};"></div>
                <span style="font-size:14px; color:{TEXT_SECONDARY};">{s}</span></div>""", unsafe_allow_html=True)
    with tab_t:
        cols = st.columns(2)
        for i, t in enumerate(info["treatment"]):
            with cols[i % 2]:
                st.markdown(f"""<div class="symptom-item">
                ...
                <span style="font-size:14px; color:{TEXT_SECONDARY};">{t}</span></div>""", unsafe_allow_html=True)
    with tab_st:
        c1, c2 = st.columns(2)
        with c1:
    ...
    <div style="background-color:{BG_SECONDARY}; border:1px solid {BORDER}; border-radius:12px;
        ...
        <p style="font-size:12px; color:{TEXT_MUTED}; margin:0; line-height:1.6;">
            This tool is for educational and research purposes only. Always consult a qualified healthcare provider.
        </p>
    </div>""", unsafe_allow_html=True)
...
def main():
    ...
    <div style="border-top:1px solid {BORDER}; margin-top:40px; padding-top:20px; text-align:center;">
        <p style="font-size:12px; color:{TEXT_MUTED};">
            Brain Tumor MRI Classifier &bull; Powered by EfficientNet-B3 + Grad-CAM &bull; Educational use only
        </p>
    </div>""", unsafe_allow_html=True)
